...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if ((v === "--height" || v === "-h") && next) {
      a.height = Number(next);
      i++;
      continue;
    }
    if (v === "--vol-height" && next) {
      a.volHeight = Math.max(2, Number(next));
      i++;
      continue;
    }
    if (v === "--ma" && next) {
      a.ma = Math.max(1, Number(next));
      i++;
      continue;
    }
    if (v === "--ma2" && next) {
      a.ma2 = Math.max(1, Number(next));
      i++;
      continue;
    }
    if (v === "--help") {
...
async function main() {
  ...
  if (candles.length === 0) {
    warn(
      `[plot] Aucune candle trouvée pour ${args.pair} sur la période demandée.`,
    );
    return;
  }
  ...
  if (closesView.length < 2) {
    warn(
      `[plot] Trop peu de points à tracer (closes=${closesView.length}). Réduis --ma/--ma2 ou augmente --days/--limit.`,
    );
    return;
  }

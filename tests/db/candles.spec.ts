...
describe("Candles DB", () => {
  ...
  it("upsert des bougies", () => {
    ...
    let candles = getCandles(
      "XBT/EUR",
      lastTime ?? Math.floor(Date.now() / 1000) - 28800,
      10,
    );
    expect(candles.length).toEqual(1); // 2 bougies existantes
    upsertCandles("XBT/EUR", [
      {
        time: lastTime ?? Math.floor(Date.now() / 1000),
        open: 100,
        high: 110,
        low: 90,
        close: 105,
      ...
      {
        time: (lastTime ?? Math.floor(Date.now() / 1000)) + FOUR_HOURS_SECS,
        open: 105,
        high: 115,
        low: 95,
        close: 110,
      ...
      {
        time: (lastTime ?? Math.floor(Date.now() / 1000)) + FOUR_HOURS_SECS * 2,
        open: 105,
        high: 115,
        low: 95,
        close: 110,

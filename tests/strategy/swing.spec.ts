...
async function loadSwingWithMocks(opts: {
  ...
  const getOpenTradesMock = vi.fn().mockImplementation((pair: string) =>
...
describe('swing.runSwingForPairOnce', () => {
  ...
  it('HOLD/NOT_ENOUGH_HISTORY quand bougies < need', async () => {
    ...
    const params = { maShort: 2, maLong: 3, lookback: 50, mode: 'simulation' as const };
  ...
  it('HOLD/NO_BUDGET quand le budget restant est 0', async () => {
    const { swing } = await loadSwingWithMocks({
      ...
      openTrades: [
        { pair: 'ETH/EUR', status: 'OPEN', invested: 100 },
      ],
      pairSettings: {
        max_invest_fiat: 100,
        max_per_tx_fiat: 20,
  ...
  it('SELL (mode réel) sur croisement baissier confirmé (closeTrade appelé)', async () => {
    ...
    const params = { maShort: 2, maLong: 3, lookback: 50, mode: 'real' as const };
    ...
    expect(((mocked.closeTrade as any).mock.calls[0][1] as any).exit_price).toBe(0);
  });
});
